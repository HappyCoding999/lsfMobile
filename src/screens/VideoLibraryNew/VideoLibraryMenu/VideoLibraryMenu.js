...
class VideoLibraryMenu extends React.Component {
  // static router = VideoLibrary.router;
  ...
  render() {
    ...
    var App = createMaterialTopTabNavigator(
      {
        WORKOUTS: {
          screen: (screenProps) => (
            <VideoLibrary
      ...
      },
      {
        tabBarOptions,
        initialRouteName:
          this.props.screenProps.selected_tab_index === 0
            ? irn_w
            : this.props.screenProps.selected_tab_index === 1
            ? irn_r
            : this.props.screenProps.selected_tab_index === 2
            ? irn_h
            : irn_y,
      }
    );
...
export default connect(mapStateToProps)(VideoLibraryMenu);
